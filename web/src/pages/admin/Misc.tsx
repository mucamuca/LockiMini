...
function CouponDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center p-4" role="dialog" aria-modal="true">
      ...
      <form onSubmit={onSubmit} className="relative w-full max-w-md animate-fade-up rounded-2xl bg-white p-6 shadow-2xl">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-ink-900">Novo cupom</h2>
          ...
          <div className="flex rounded-xl border border-ink-200 p-1">
            <button
              ...
              className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium transition ${
                type === 'percent' ? 'bg-ink-900 text-white' : 'text-ink-600'
              }`}
            >
              Percentual
            </button>
            <button
              ...
              className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium transition ${
                type === 'amount' ? 'bg-ink-900 text-white' : 'text-ink-600'
              }`}
            >
              Valor fixo
            </button>
              ...
              <tbody className="divide-y divide-ink-100">
                {customers.map((c) => (
                  <tr key={c.id} className="hover:bg-ink-50/60">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="truncate font-medium text-ink-900">
                            {c.name}
                            {c.role === 'ADMIN' && (
                              <span className="ml-2 chip bg-brand-50 text-brand-700 ring-1 ring-brand-200">
                                admin
                              </span>
                            )}
                          </p>
                    ...
                    <td className="px-4 py-3 text-ink-600">{dateTime(c.createdAt)}</td>
                    <td className="px-4 py-3 text-center text-ink-600">{c.orderCount}</td>
