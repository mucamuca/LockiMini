...
export function AdminDashboard() {
  ...
  return (
    <>
      <PageHeading
        ...
        actions={
          <div className="flex rounded-xl border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 p-1">
            {RANGES.map((r) => (
              <button
                key={r.days}
                onClick={() => setDays(r.days)}
                className={`rounded-lg px-3 py-1.5 text-sm font-medium transition ${
                  days === r.days ? 'bg-ink-900 text-white' : 'text-ink-600 dark:text-ink-300 hover:bg-ink-50 dark:hover:bg-ink-850'
                }`}
              >
                {r.label}
              </button>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {kpis.pendingPayment > 0 && (
            <Link
              ...
              className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 dark:bg-amber-950/40 p-4 transition hover:border-amber-300"
            >
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-3">
        <section className="card p-6 xl:col-span-2">
          <div className="mb-5 flex items-baseline justify-between">
            <h2 className="text-base font-bold text-ink-900 dark:text-ink-50">Receita por dia</h2>
            <span className="text-sm text-ink-500 dark:text-ink-400">{money(kpis.revenueCents)} no periodo</span>
          </div>
          <BarChart
        ...
        <section className="card p-6">
          <h2 className="text-base font-bold text-ink-900 dark:text-ink-50">Estoque em alerta</h2>
          {data.lowStock.length === 0 ? (
            <p className="mt-4 text-sm text-ink-500 dark:text-ink-400">Nenhum produto abaixo do minimo. Tudo certo.</p>
          ) : (
  ...
  loading="lazy"/>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-3">
        <section className="card p-6 xl:col-span-2">
          <h2 className="mb-4 text-base font-bold text-ink-900 dark:text-ink-50">Pedidos recentes</h2>
              ...
              <tbody className="divide-y divide-ink-50">
                {data.recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-ink-50/60">
                    ...
                    <td className="py-3 text-ink-600 dark:text-ink-300">{order.email}</td>
                    <td className="py-3">
                      <StatusBadge status={order.status} />
                    </td>
                    <td className="py-3 text-right font-semibold text-ink-900 dark:text-ink-50">
        ...
        <section className="card p-6">
          <h2 className="mb-4 text-base font-bold text-ink-900 dark:text-ink-50">Mais vendidos</h2>
          {data.topProducts.length === 0 ? (
            <p className="text-sm text-ink-500 dark:text-ink-400">Sem vendas no periodo.</p>
          ) : (
            <ol className="space-y-3">
              {data.topProducts.map((p, i) => (
                <li key={p.productId} className="flex items-center gap-3">
                  <span className="grid h-6 w-6 shrink-0 place-items-center rounded-lg bg-ink-100 dark:bg-ink-800 text-xs font-bold text-ink-600 dark:text-ink-300">
                    {i + 1}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-ink-900 dark:text-ink-50">{p.name}</p>
                    <p className="text-xs text-ink-500 dark:text-ink-400">{p.units} un. vendidas</p>
                  </div>
                  <span className="text-sm font-semibold text-ink-900 dark:text-ink-50">{money(p.revenueCents)}</span>
                </li>
              ))}
            </ol>
          )}
...
function Kpi({
  ...
  return (
    <div className="card p-5">
      ...
      <p className="mt-2 text-2xl font-bold tracking-tight text-ink-900 dark:text-ink-50">{value}</p>
      {change !== null && change !== undefined ? (
        <p
          className={`mt-1.5 flex items-center gap-1 text-xs font-semibold ${
            positive ? 'text-emerald-600' : 'text-rose-600 dark:text-rose-400'
          }`}
        >
